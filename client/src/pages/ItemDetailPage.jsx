...
export default function ItemDetailPage() {
  ...
  if (isError) {
    return (
      <div className="max-w-3xl mx-auto p-6 text-red-600">{error?.message || 'Failed to load item'}</div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow">
      ...
      <nav className="text-sm mb-4 text-gray-600">
        ...
        <span className="text-gray-800">{item.title || t('item.detail', { defaultValue: 'Item Details' })}</span>
      </nav>

      <div className="flex items-start justify-between mb-4">
        <h1 className="text-2xl font-semibold text-gray-900">{item.title || t('item.detail', { defaultValue: 'Item Details' })}</h1>
        <button
          onClick={onLike}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          ...
          {inventoryId ? (
            <Link to={`/inventories/${inventoryId}`} className="text-blue-600 hover:underline">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600">
          ...
          {item.customId || item.serial ? (
            <span className="inline-flex items-center gap-2">
              <code className="px-2 py-0.5 rounded bg-gray-100 text-gray-800">{item.customId || item.serial}</code>
              <span
                role="button"
                tabIndex={0}
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    copyToClipboard(item.customId || item.serial);
                    e.preventDefault();
                  }
                }}
                className="inline-flex cursor-pointer p-1 rounded hover:bg-gray-100"
              >
      ...
      {Array.isArray(item.tags) && item.tags.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {item.tags.map((tg) => (
              <span key={tg} className="px-2 py-1 text-xs rounded bg-gray-100 text-gray-800">{tg}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {(() => {
        ...
        return (
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-2">{t('fields.customFields', { defaultValue: 'Custom Fields' })}</h2>
                ...
                <div key={idx} className="text-sm text-gray-700">
                  ...
                  {r.type === 'boolean' ? (
                    ...
                    isImageUrl(r.value) ? (
                      ...
                      <a href={r.value} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline break-all">{r.value}</a>
                    )
                  ) : (
                    String(r.value)
                  )}
      ...
      <div className="flex items-center gap-4">
        {inventoryId ? (
          <Link to={`/inventories/${inventoryId}`} className="text-blue-600 hover:underline">{t('nav.backToInventory', { defaultValue: 'Back to Inventory' })}</Link>
        ) : (
          <Link to="/inventories" className="text-blue-600 hover:underline">{t('nav.inventories', { defaultValue: 'Inventories' })}</Link>
        )}
        <Link to="/" className="text-blue-600 hover:underline">{t('nav.home', { defaultValue: 'Home' })}</Link>
      </div>
    </div>
  );
}
